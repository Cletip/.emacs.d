...
Emacs en mode serveur permet de gagner beaucoup de temps : lors de l'ouverture du premier Emacs, on pourra, pour les prochaines ouvertures, appeler un client qui se connecte au serveur/au premier emacs. Le serveur peut être kill comme un Emacs normal
De plus, on peut aussi run emacs en mode daemon. La seule différence est que c'est en arrière plan.

Ce qui est bien c'est qu'avec le serveur, cela marche sur toutes les platformes.
