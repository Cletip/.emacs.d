...
***** Pour automatiquement commit quand on rajoute un document (plus besoin, git-autocommit-mode)
...
****** Quand on-change le titre, ça change le nom du fichier
...
#+begin_src emacs-lisp :tangle no
  ...
  ;; (add-hook 'after-save-hook 'org-roam-rename-file-with-new-title)
  ;; (remove-hook 'after-save-hook 'org-roam-rename-file-with-new-title)
...
#+end_src
****** Permet de rename un noeud partout !
...
****** Fonction pour naviguer rapidement au niveau des noeuds
...
#+begin_src emacs-lisp :tangle no
  ...
  ;; marche pas, car vulpea-note pas un hash
  
  (cp/vulpea-transform-title-links (cp/vulpea-links-to))
  (defun cp/vulpea-transform-title-links(liste)
    "renvoie liste de note avec noté devant \"Links :\""
    (dolist (note liste)
      (message "la note : %s" (vulpea-note-title note))
      ;; (puthash "title"
      ;; (concat "Back : " (gethash "title" note))
      ;; note)
      )
    )
...
****** Fonction permettant de vérifier si un titre possède un id
