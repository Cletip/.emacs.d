...
*** Modification de vulpea insert pour choix des aliases
...
***** TODO Couleur des liens dans org-mode en fonction du type de lien
:LOGBOOK:
- State "TODO"       from              [2023-02-14 Tue 11:10]
:END:
...
***** Pour que les tables soient affiché correctement
...
***** Quand on créer des liens sur un heading, on veut créer un id !
...
3. ajouter la ligne avec le bon lien ! #+transclude: [[id:20220621120442360949][test contac]] :lines 0-20
...
***** TODO La valeur du tag
:LOGBOOK:
- State "TODO"       from              [2023-02-15 Wed 15:01]
:END:
...
***** Nom des dossier pour attachment
...
***** CANCELED On créer les dossier par années/mois/jour/lerest par rapport à la date de la création du noeud pour les rangements (ancien) 
...
***** TODO Pour prendre des notes sur des fichiers
:LOGBOOK:
- State "TODO"       from              [2023-02-12 Sun 11:04]
:END:
...
***** Pour rechercher mes fichiers

Normalement, chaque fichier/dossier est relié à une note/heading avec un id. Comme cela, je peux bouger les heading / les fichiers sans problème.

****** TODO Avec les notes (filtrer les notes avec tags "attached")
:LOGBOOK:
- State "TODO"       from              [2023-02-15 Wed 14:48]
:END:

****** Sans les notes, les fichiers pur
:LOGBOOK:
- State "TODO"       from              [2023-02-15 Wed 14:48]
:END:
...
**** Gestion de l'intéraction avec le pc / de capture de page
...
***** TODO Grasp (plus simple ?)
:LOGBOOK:
- State "TODO"       from              [2023-02-23 Thu 18:00]
:END:
...
****** Pour avoir un mot de passe hors d'emacs
...
#+begin_src sh
  ...
  emacsclient -e "(efs/lookup-password :host \"clement020302Gmail\")" | cut -d '"' -f2
#+end_src
...
****** TODO Savoir si une note est populaire en fonction de son nombre de visite
:LOGBOOK:
- State "TODO"       from              [2023-02-19 Sun 21:49]
:END:

pour savoir le nombre de fois où c'est cité, suffit de savoir le nombre de backlinks
. mais pour savoir si une note est vraiment populaire, il faut savoir combien de fois je la visite. il faut donc que je store cela à un endroit (propriété org-mode). comment savoir quand je visite un node ? avec son "atime" dans la base de données ! ou autrement ? avec un "find-file-hook" !
...
***** Autre preview des noeuds, pas utilisé
...
***** TODO Fonction modifié pour des liens autre que "id" : les liens avec relations
:LOGBOOK:
- State "TODO"       from              [2023-02-11 Sat 01:42]
:END:
...
[[id:lien][description]]
en
[[id:lien::relation::autreRelation][description]]

où lien, relation et autreRelation sont des ids
#+end_example

Car il faut garder l'ancienne structure ! Donc conserver "id" et pas faire autre chose.

Pour cela, il ne faut pas modifier les choses côté org-mode, mais seulement org-roam.
Je voulais modifier org-id-update-id-locations, mais enfait cela fais juste le lien entre un :ID: et un fichier.

Créer une table qui stocke les relations. Pour les avoirs, suffit de parser les liens, avec l'id récupérer sur la note vulpea permettant d'avoir le node org-roam et donc les fameux liens.

*Tout ceci est une bonne idée... le problème, c'est que cela ne marche pas vraiment, car ne correspond pas au type "id". Il faudrait mettre mon parseur partout, dans tout org-mode (lors d'un click sur un lien, lors d'un export etc). Il faudrait aussi tester si, à chaque fois, c'est un id simple ou bien un id avec relation* 

Je pense donc qu'il faut créer un nouveau type de lien, exclusif à org-roam en utilisant les fonctionnalité d'org-mode.

****** Version 1 : modifier org-mode

id1:id2

******* Dans org-mode pour ajouter un type de tel sorte que cela soit détectecter comme un "type id"

******** La regex pour check si li type peut-être un id
...
******** Fonction pour parser le lien, il ne faut plus que cela soit "fuzzy", qui est le cas par défaut quand il trouve pas
...
******** Fonction pour open avec le lien
:LOGBOOK:
- State "DONE"       from "TODO"       [2023-02-11 Sat 17:12]
- State "TODO"       from              [2023-02-11 Sat 17:12]
- State "TODO"       from              [2023-02-11 Sat 16:04]
:END:
...
Pour l'export, il va falloir "voir chaque cas". Par exemple, en html, il suffit de passer le lien "normal" dans la fonction "org-export-resolve-id-link". Il faut donc faire un truc sur le id-rel, et le "convertir" en lien id.

Pour ces fonction, faire des advice-add avec des :filter-args ?

******** TODO Html
:LOGBOOK:
- State "TODO"       from              [2023-02-11 Sat 17:31]
:END:

Je sais déjà où faire la moddif sur cette fonction pour que cela marche bien : rechercher  ";; TODO"
...
******* CANCELED La db pour stocker tout cela (temporaire avec vulpea, je vais pas aller rajouter une colonne dans celle d'org-roam... quoique). Au final, inutile car je fais les choses du type id1:id2, donc pas besoin de rajouter quelque chose au niveau de la dblocks
:LOGBOOK:
- State "TODO"       from              [2023-02-11 Sat 02:39]
:END:

****** TODO Version 2 : modifier org-roam (privilégier celle la ?)
:LOGBOOK:
- State "TODO"       from              [2023-02-25 Sat 13:59]
:END:


            
******* TODO Création du type de lien dans org-mode
:LOGBOOK:
- State "TODO"       from              [2023-02-25 Sat 14:01]
:END:

To avoid confusion and ensure that your relation information is properly stored and accessible, it's recommended to use the double colon format [[id-rel:destination::relation]].
...
******* Modifier la fonction pour insérer des liens dans la db (pour insérer dans la table lien :type = id du lien, et valeur = id de la note)
...
******* TODO Modification pour avoir les backlinks en entier
:LOGBOOK:
- State "TODO"       from              [2023-02-25 Sat 14:01]
:END:

******** Modifier la fonction backlinks-get : NON ! car il faudrait créer une nouvelle section pour les backlinks de type id-rel, avec affichage par exemple : relation-liens
   ...
   When UNIQUE is t, limit to unique sources."
    (let* ((sql (if unique
                  ...
                  [:select [source dest pos properties]
                           ...
                           :where (= dest $s1)
                           :and (= type "id")]))
  ;; rajouter ici "id-rel", il faut prendre à la fois 
           (sql2 (if unique
                     [:select :distinct [source dest pos properties]
                              :from links
                              :where (= dest $s1)
...
******** Modifier la section des backlinks

créer une nouvelle section dans la variable
...
s'inspirer des section déjà existante : org-roam-backlinks-section par exemple

****** Mon "parser" de fonction pour run une fonction par rapport à l'id


******* Fonction pour run une liste de fonction sur chaque liens
(org-roam-db-map-links (list #'org-roam-db-insert-link #'une-autre-fonction))

******* En fonction de l'id, faire tel fonction

#+begin_src emacs-lisp :tangle no
  (defun cp/translate-id-to-function ()
    (interactive)
    (let* ((link (org-element-context))
      ...
      ;; (funcall
      ;; (message "%s" path)
      (message "%s" (intern (concat "cp/" path)))
      ;; )
      ))
...
[[id-rel:val1::val2][machin descriptior]]

****** Mise à jour de la db pour 
...
****** Ancien, à supprimer petit à petit

******* Db pour stocker les relations
...
******* Insérer le bon liens dans la db d'org-roam
...
******* Inutile : Nouvelle fonction de trie qui compare les type de noeuds

Utile ?

#+begin_src emacs-lisp :tangle no
  (defun org-roam-backlinks-sort-by-type (a b)
...
******* La section dans le buffer des backlinks

Devoir parcourir pour chaque relation et écrire 
...
******* Récupération backlinks autre que seulement "id"
...
** TODO org-edna, a intégrer avec le reste
:LOGBOOK:
- State "TODO"       from              [2023-02-14 Tue 18:05]
:END:
...
** TODO Recettes
:LOGBOOK:
- State "TODO"       from              [2023-02-14 Tue 20:20]
:END:

Principe des recettes : une note aura la recette X, puis un transclude sur une autre note "ingrédient de la recette X", et un liens vers la préparation de la recette Y. Pour modifier le nombre de personne, utiliser les fonction de org-transclusion-live-sync-start et org-transclusion-live-sync-exit, avec org-chef-edit--do-replace sur la région qui permet de multiplié par un coef. Changer la fonction et les fonctions dans org-chef-edit.el pour cela (notamment comment on récupère le nombre de personne de base + *ne pas prendre en compte les ids*), et automatiser ceci.

donc :
title: ingrédients de machin

- pas obligatoire une liste
# - 2 [type:ingrédient][patate]
- etc

types de liens pour pouvoir faire être databasable !
...
#+begin_src emacs-lisp :tangle no
  (use-package pq :straight (pq :type git :host github :repo "anse1/emacs-libpq"))
#+end_src
...
#+begin_src emacs-lisp :tangle no
  (add-to-list 'load-path "/home/utilisateur/Téléchargements/rcd-notes")
  (require 'rcd-cf)
#+end_src
...
(peut-être cela aussi : python3 -m pip install pip setuptools --upgrade)
2. Start App
khoj
...
** Télécharger des images simplement

*** TODO Org-download (toutes une image)
:LOGBOOK:
- State "TODO"       from              [2023-02-21 Tue 22:01]
:END:
...
[[https://github.com/LionyxML/ros][ici]]
